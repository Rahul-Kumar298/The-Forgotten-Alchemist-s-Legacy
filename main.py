...
if c1 == "y":
    ...
    speak("You accept the challenge.")
    print()
    print("Now , You have to decide where you want to go first.")
    speak("Now , You have to decide where you want to go first.")
    ...
    if c2 == 1:
        ...
        placego =f"You decided to go to {chosen_place}, Your are going to {list(place.keys())[c2 - 1]}."
        print(placego)
        speak(placego)
        print()
        from fire_chamber import play_flame_symbol_puzzle,play_temperature_control_test, blazing_bridge_trap
                # Main game logic
        task_completed = 0
        ...
        speak(f"You have completed {task_completed} tasks out of 20 tasks to get fire elemental key.")
        
        
    elif c2 == 2:
        chosen_place = place_list[c2 - 1]
        placego =f"You decided to go to {chosen_place}. \nYour are going to {list(place.keys())[c2 - 1]}."
        print(placego)
        speak(placego)
